...
function validateTXNAMT(txnamt) {
  ...
  // Check if it's a positive number (allowing decimals)
  if (!/^[0-9]+(\.[0-9]+)?$/.test(txnamtStr)) {
    return { isValid: false, error: 'E4', message: 'Please check TXNAMT' };
  }

  // Optionally, you can set a max length (e.g., up to 13 chars including decimal point before formatting)
  if (txnamtStr.length > 13) {
    return { isValid: false, error: 'E4', message: 'Please check TXNAMT' };
  }
